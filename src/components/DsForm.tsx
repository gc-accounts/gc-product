...
const DsForm: React.FC<DsFormProps> = ({ isModal = false, onClose }) => {
  ...
  return (
    <Card className={`${isModal ? 'shadow-none' : 'bg-white shadow-lg rounded-2xl p-6 sm:p-8'}`}>
      {/* 🛑 CRITICAL FIX FOR RECAPTCHA CLICK-THROUGH 🛑 
         This global CSS forces the Google Challenge container (which is attached to body)
         to have a higher z-index than the Modal and, most importantly, forces pointer-events to work.
      */}
      <style jsx global>{`
        /* Target the Google ReCAPTCHA Challenge Container */
        body > div[style*="position: absolute"][style*="z-index"] {
            z-index: 2147483647 !important; /* Maximum safe integer */
            pointer-events: auto !important;
        }
        
        /* Target the iframes specifically */
        ...
        /* Ensure the bubble arrow is also visible */
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 relative">
            <div className="relative">
              ...
              {showDropdown && countrySearch.length > 0 && (
                <ul className="absolute z-[100] bg-white border border-gray-200 rounded-md mt-1 w-full max-h-48 overflow-y-auto shadow-lg">
