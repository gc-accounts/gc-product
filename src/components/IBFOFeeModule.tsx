'use client'
import React from 'react'
import { Card, CardContent } from './ui/card';
import { CheckCircle } from 'lucide-react';
import Link from 'next/link';
import { Button } from './ui/button';

 const cohorts = [
      {
        startDate: "27 Dec 2025",
        status: "Open",
        color: "bg-primary-green",
        spots: "Available: 12/20"
      },
      { 
        startDate: "31 Jan 2026", 
        status: "Limited", color: "bg-accent-gold", spots: "Available: 16/30" },
    ];

interface Props {
  sectionClass?: string;
  title?: string;
  subText?: string;
  currency?:string;
  basePrice?:number;
}
const IBFOFeeModule = ({ sectionClass, title, subText, currency, basePrice }: Props) => {


  return (
    <section className={`${sectionClass ? sectionClass : ''}`}>
      <div className="container max-w-7xl mx-auto">

        <div className="text-center mb-8 lg:mb-10 max-w-8xl">
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-dark-gray mb-2 lg:mb-3">
            {title}
          </h2>
          <p className="text-sm md:text-base text-medium-gray mx-auto leading-relaxed">
            {subText}
          </p>
        </div>

          {/* Main Pricing Card */}
          <div className="mx-auto mb-12 lg:mb-16 max-w-4xl">
            <Card className="bg-white shadow-lg border-0 relative overflow-hidden">
              <CardContent className="p-8 lg:p-12">
                <h3 className="text-2xl lg:text-3xl font-bold text-dark-gray mb-6 lg:mb-8 text-center">
                  Investment Banking & Finance Operations Bootcamp
                </h3>

                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4 mb-8">
                  {[

                    "12 modules covering all key areas of banking and finance operations",
                    "3+ hands-on projects and live simulations",
                    "Resume and LinkedIn profile guidance",
                    "Mentorship sessions by finance professionals",
                    "Interview preparation and placement assistance",
                    "Dual Certification",
                  ].map((feature, index) => (
                    <div key={index} className="flex items-start space-x-3">
                      <CheckCircle className="w-5 h-5 text-primary-green shrink-0 mt-0.5" />
                      <span className="text-dark-gray text-sm leading-relaxed">{feature}</span>
                    </div>
                  ))}
                </div>


                <div className="text-center mb-2">
                  <div className="flex items-center justify-center space-x-2 mb-3">
                    <span className="text-red-500 line-through text-lg">
                      {/* ₹15,000 */}
                      {currency === 'INR' ? '₹15,000' : '$250'}
                    </span>
                    <span className="text-red-500 line-through text-lg">
                      + GST
                    </span>
                  </div>
                  <div className="flex items-center justify-center space-x-2 mb-3">
                    <span className="text-4xl lg:text-5xl font-bold text-primary-green">
                     
                     {currency === 'INR' ? '₹5,000' : `$${basePrice}`}
                    </span>
                    <span className="text-xl lg:text-2xl font-bold text-primary-green">
                      + GST
                    </span>
                  </div>
                </div>

                <p className='mb-8 text-center text-secondary-green text-sm'>(Limited-time introductory offer)</p>

                <div className="space-y-3">
                  <Link href={'/course-checkout/investment-banking-bootcamp'}>
                    <Button className="w-full bg-yellow-1 hover:bg-yellow-2 px-8 py-4 text-lg font-semibold cursor-pointer">
                      Enroll Now
                    </Button>
                  </Link>
                  {/* <Button variant="outline" className="w-full border-2 border-primary-green text-primary-green hover:bg-primary-green hover:text-white py-3 text-lg font-semibold h-12">
                    Enquire Now
                  </Button> */}
                </div>
              </CardContent>
            </Card>
          </div>

          {/* Cohort Dates */}
          <div className="text-center mb-8 lg:mb-6">
            <h3 className="text-2xl lg:text-3xl font-bold text-dark-gray mb-2 lg:mb-2">
              Upcoming Cohorts
            </h3>
            {/* <p className="text-medium-gray leading-relaxed">
              Choose a cohort date that works best for you
            </p> */}
          </div>

          <div className="md:flex justify-center md:max-w-[550px] max-w-full mx-auto md:space-y-0 space-y-4">
            {cohorts.map((cohort, index) => (
              <Card key={index} className={`${cohort.color} text-white border-0 hover:shadow-lg transition-all duration-300 h-full w-fit mx-auto`}>
                <CardContent className="p-6 text-center h-full flex flex-col justify-center">
                  <div className="bg-white text-primary-green px-3 py-1 rounded-full text-xs font-semibold mb-4 inline-block">
                    {cohort.status}
                  </div>
                  <div className="text-lg font-bold mb-2">
                    Start: {cohort.startDate}
                  </div>

                  <div className="text-xs opacity-90 leading-relaxed">
                    {cohort.spots}
                  </div>
                </CardContent>
              </Card>
            ))}
          </div>
        </div>
      </section>
    );
  };
export default IBFOFeeModule