...
export interface FieldConfig {
  ...
  accept?: string; // For file types (e.g., 'image/*', '.pdf,.docx')
}
...
const DynamicForm: React.FC<DynamicFormProps> = ({
  ...
  // Use useMemo to filter countries only when the search term changes
  ...
  const handleCountrySelect = useCallback((country: typeof CountryCodeData[0]) => {
  ...
  const handleCountryInputBlur = useCallback(() => {
    // A small delay to allow for click events on the dropdown items
    setTimeout(() => setShowCountryDropdown(false), 200);
  }, []);

  const countryField = useMemo(() => fields.find(field => field.name === 'countryCode'), [fields]);
  const phoneField = useMemo(() => fields.find(field => field.name === 'phone'), [fields]);
    ...
    <Form {...form}>
      <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
        ...
        <Button
          ...
          className="w-full mt-6 flex items-center justify-center rounded-md"
        >
